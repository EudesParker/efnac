...
SELECT h.id_facture AS Num, u.nom, u.prenom, h.type, h.prix_total, h.quantite, h.date_obtention
FROM historique h, utilisateur u
WHERE u.id_utilisateur = h.id_utilisateur
ORDER BY h.id_facture, u.nom;

SELECT h.id_facture AS Num, u.nom, u.prenom, h.type, h.prix_total, h.quantite, o.designation, h.date_obtention
FROM historique h, utilisateur u, objet o
WHERE u.id_utilisateur = h.id_utilisateur
AND o.id_objet = h.id_objet
ORDER BY h.id_facture, u.nom;

SELECT u.id_utilisateur, u.civilite, u.nom, u.prenom, u.email, u.mot_de_passe, u.IP, u.dateinscription, u.telfixe, u.telmobile, d.designation
FROM utilisateur u, droits d
WHERE u.id_droit = d.id_droit
ORDER BY nom;

SELECT u.nom, u.prenom, COUNT(h.id_utilisateur) AS NbCommande
FROM utilisateur u, historique h
WHERE u.id_utilisateur = h.id_utilisateur;

SELECT MAX(id_utilisateur) as nbTT FROM historique;

SELECT u.nom FROM utilisateur u WHERE MAX(id_utilisateur) as nbTT FROM historique;

SELECT COUNT(*) FROM historique

SELECT u.nom, u.prenom(COUNT(id_utilisateur)) as nbTT FROM historique;

SELECT u.nom, u.prenom, h.id_utilisateur AS NbCommande
FROM utilisateur u, historique h
WHERE u.id_utilisateur = h.id_utilisateur
AND h.id_utilisateur = (SELECT MAX(COUNT(id_utilisateur)) FROM historique);

//MARCHE !!!!!
SELECT COUNT(id_utilisateur) AS NbTT FROM historique GROUP BY id_utilisateur;
SELECT u.nom, u.prenom, COUNT(h.id_utilisateur) AS NbTT FROM utilisateur u, historique h
WHERE u.id_utilisateur = h.id_utilisateur GROUP BY h.id_utilisateur;

create view maxV as(SELECT u.nom, u.prenom, COUNT(h.id_utilisateur) AS NbTT FROM utilisateur u, historique h
WHERE u.id_utilisateur = h.id_utilisateur GROUP BY h.id_utilisateur);
select max(nbTT) from maxV;

MARCHE !!!!! OK
SELECT u.nom, u.prenom, COUNT(h.id_utilisateur) AS NbTT FROM utilisateur u, historique h
WHERE u.id_utilisateur = h.id_utilisateur GROUP BY h.id_utilisateur
ORDER BY NbTT DESC LIMIT 0,1;

SELECT u.nom, u.prenom, MAX(h.id_utilisateur) AS NbTT FROM utilisateur u, historique h
WHERE h.id_utilisateur = (SELECT COUNT(h.id_utilisateur) FROM historique h GROUP BY h.id_utilisateur);

SELECT MAX(COUNT(h.id_utilisateur)) AS NbTT FROM historique h GROUP BY h.id_utilisateur;

SELECT u.nom, u.prenom, COUNT(h.id_utilisateur) AS NbTT FROM utilisateur u, historique h
WHERE h.id_utilisateur = (SELECT MAX(h.id_utilisateur) FROM historique)
AND u.id_utilisateur = h.id_utilisateur GROUP BY h.id_utilisateur;

SELECT u.nom, u.prenom, MAX(h.id_utilisateur) FROM utilisateur u, historique h
WHERE h.id_utilisateur = SELECT COUNT(h.id_utilisateur) AS NbTT FROM historique GROUP BY h.id_utilisateur;
SELECT nom, qte FROM Vue_Clients WHERE qte >= (SELECT MAX(qte) FROM Vue_Clients);

SELECT u.nom, u.prenom, MAX(COUNT(h.id_utilisateur)
FROM utilisateur u, historique h
WHERE u.id_utilisateur = h.id_utilisateur;

SELECT MAX(CommandeTT)
FROM (SELECT id_utilisateur, COUNT(id_utilisateur) CommandeTT
FROM historique
...
INSERT INTO Eudes VALUES ('', 'Eudes', 'Kara');

INSERT INTO kevin VALUES ('', 'Eudes', 'Kara');

GRANT = accorder !!!

GRANT SELECT, INSERT ON Trelis.* TO kevin IDENTIFIED BY "azerty";

GRANT ALL PRIVILEGES ON *.* TO benmichel IDENTIFIED BY "azerty";

REVOKE INSERT ON Trelis.* FROM Vianney;